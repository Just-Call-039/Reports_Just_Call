...
client_click.execute("alter table otchet_25 delete where date(my_date) = date(now());")
client_click.execute("alter table request_25 delete where date(date_reguest) = date(now());")
time.sleep(120)
...
now_time = time.time()
to_req = r'C:\Users\Supervisor031\Отчеты\Отчет_25\Files\Temp_request.csv'
...
t_calls = pd.read_csv(to_calls, sep=';', dtype='unicode')
t_req = pd.read_csv(to_req, sep=';', dtype='unicode')
...
clear_calls = pd.merge(new_calls, t_calls, left_on=['phone', ('my_date', 'last')], right_on=['phone', 'my_date'],
                       how='left')
clear_calls.to_csv(r'C:\Users\Supervisor031\Отчеты\Отчет_25\Files\Clear_calls.csv', index=False, sep=';',
                   encoding='utf-8')
...
day_req.rename(columns={'my_phone_work': 'phone_number', 'status': 'status_request', 'date': 'date_reguest'},
               inplace=True)
day_req.to_csv(r'C:\Users\Supervisor031\Отчеты\Отчет_25\Files\My_request.csv', index=False, sep=';', encoding='utf-8')
