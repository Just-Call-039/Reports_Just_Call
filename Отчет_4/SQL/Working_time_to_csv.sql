...
from (select 'id_user',
      ...
      select id_user,
             ...
             if((fact - talk_inbound -
                 ...
                 obrabotka_in_fact - progul_obrabotka_in_fact) is NULL, 0, (fact - talk_inbound -
                                                                            if(
                                                                                        (talk_outbound - (recall - recall_talk)) <
                                                                                        0,
                                                                                        0,
                                                                                        (talk_outbound - (recall - recall_talk))) -
                                                                            obrabotka_in_fact -
      ...
      where date(date) between date(now()) - interval 70 day and date(now()) - interval 1 day
        and id_user not in ('1', '')
        and id_user is not null) as t_1
into outfile '/var/lib/mysql-files/4_report/Working_time.csv'
